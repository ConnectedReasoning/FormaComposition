...
the CLI takes one piece.json per piece now instead of a shared theme.json
plus piece file(s).
"""
import json
import subprocess
...
class TestValidInputRuns:
    def test_single_piece_generates_and_exits_zero(self, main_py, piece_path, tmp_path):
        out = tmp_path / "out.mid"
        result = _run(main_py, piece_path, "--output", str(out))
        assert result.returncode == 0
        assert out.exists()
        assert out.stat().st_size > 0

    def test_success_line_reports_title_and_output_path(self, main_py, piece_path, tmp_path):
        out = tmp_path / "out.mid"
        result = _run(main_py, piece_path, "--output", str(out))
        assert "Test Piece" in result.stdout
        assert str(out) in result.stdout

    def test_batch_generation_with_outdir_reports_summary(self, main_py, piece_path, tmp_path):
        album_dir = tmp_path / "album"
        result = _run(main_py, piece_path, piece_path, "--outdir", str(album_dir))
        assert result.returncode == 0
...
class TestInvalidInputExitsCleanly:
    def test_missing_piece_file_exits_one_with_clear_message(self, main_py, tmp_path):
        result = _run(main_py, str(tmp_path / "does_not_exist.json"))
        assert result.returncode == 1
        assert "piece file not found" in result.stderr

    def test_output_flag_with_multiple_pieces_is_rejected(self, main_py, piece_path):
        result = _run(main_py, piece_path, piece_path, "--output", "x.mid")
        assert result.returncode == 1
    ...
    def test_schema_invalid_piece_reports_validation_errors_and_exits_one(
        self, main_py, tmp_path,
    ):
        bad_piece = tmp_path / "bad_piece.json"
        bad_piece.write_text(json.dumps({
            "key": "C", "mode": "ionian", "title": "Bad Piece", "sections": [],
        }))
        result = _run(main_py, str(bad_piece))
        assert result.returncode == 1
        assert "ERRORS in" in result.stdout
        assert "sections" in result.stdout

    def test_no_traceback_leaks_to_the_user_for_a_missing_file(self, main_py, tmp_path):
        """A missing-file error is an expected, handled case -- the CLI
        should print its own clear message, not a raw Python traceback."""
        result = _run(main_py, str(tmp_path / "nope.json"))
...
class TestInfoFlagDoesNotRender:
    def test_info_prints_summary_and_exits_zero(self, main_py, piece_path):
        result = _run(main_py, piece_path, "--info")
        assert result.returncode == 0
        assert "THEME:" in result.stdout
        assert "Test Piece" in result.stdout
        assert "Mode:  info" in result.stdout

    def test_info_does_not_write_a_midi_file(self, main_py, piece_path, tmp_path):
        before = set(tmp_path.iterdir())
        _run(main_py, piece_path, "--info")
    ...
    def test_info_with_outdir_still_does_not_render(self, main_py, piece_path, tmp_path):
        """--info takes priority over --outdir -- info mode never renders,
        even if an output location was also specified."""
        album_dir = tmp_path / "album"
        result = _run(main_py, piece_path, "--info", "--outdir", str(album_dir))
        assert result.returncode == 0
        assert "THEME:" in result.stdout
