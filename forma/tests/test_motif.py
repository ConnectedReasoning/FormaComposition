...
class TestGenerateDevelop:
    ...
    def test_prefer_neighbor_fold_permanently_off_regardless_of_melodic_arc(self):
        """
        REPLACES a test that pinned the old scoping decision (prefer_
        neighbor_fold on whenever melodic_arc is absent, off whenever
        present). That distinction no longer exists: prefer_neighbor_fold
        is permanently False at both motif_to_notes call sites in
        generate_develop now, regardless of arc_bias_active -- reverted
        on the composer's explicit instruction after the
        piece_broadway_boogie_v8 investigation (see the comment at
        generate_develop's motif_to_notes call for the full account).
        motif.py's generate_subject_entry keeps its own independent,
        unconditional neighbor-fold call for the original orphan-leap
        fugue bug; that fix is untouched by this revert.

        Tests the actual mechanism directly (intercepting the real kwarg
        motif_to_notes receives) rather than inferring it indirectly from
        leap statistics on one arbitrary seed -- the old test's
        with_arc/without_arc leap comparison turned out to be exactly
        that kind of indirect, seed-fragile signal: with_arc's branch was
        NEVER protected by prefer_neighbor_fold even before this revert
        (arc_bias_active already disabled it), so its leap behavior is
        unchanged either way and proves nothing about the revert; and
        without_arc's leap-or-not for any single seed is happenstance,
        not a reliable witness to whether the underlying kwarg changed.
        """
        import intervals.music.melody as melody_mod
        real_motif_to_notes = melody_mod.motif_to_notes
        seen_values = []
        ...
        def spy(*args, **kwargs):
            seen_values.append(kwargs.get("prefer_neighbor_fold"))
            return real_motif_to_notes(*args, **kwargs)
        ...
        melody_mod.motif_to_notes = spy
        try:
            generate_melody_for_progression(chords, "D", "aeolian", **kwargs)
            arc = {"apex_degree": 4, "apex_position": 0.7, "resolve_every_cycle": False}
            generate_melody_for_progression(chords, "D", "aeolian", melodic_arc=arc, **kwargs)
        finally:
            melody_mod.motif_to_notes = real_motif_to_notes

        assert seen_values, "motif_to_notes was never called -- test setup is broken"
        assert all(v is False for v in seen_values), (
            f"expected prefer_neighbor_fold=False on every call (with AND "
            f"without melodic_arc), got: {seen_values}"
        )
    ...
    def test_chained_statements_dont_collapse_toward_one_register_wall(self, monkeypatch):
        ...
        UPDATE: prefer_neighbor_fold has since been REVERTED entirely in
        generate_develop (permanently False at both motif_to_notes call
        sites -- see the comment there for the full account, including
        the piece_broadway_boogie_v8 investigation that led to reverting
        it: the composer's own ear caught a "weak on the floor" quality
        this test's original numbers didn't have language for, and the
        neighbor-fold's lack of any restoring force turned out to be the
        dominant cause once measured -- 45% of that piece's melody near a
        register wall with it active vs 40% with it off, matching a
        known-good pre-neighbor-fold render's 39% almost exactly).
        motif.py's generate_subject_entry keeps its own independent,
        unconditional neighbor-fold call for the original fugue bug --
        that fix is untouched by this revert.

        With prefer_neighbor_fold permanently off, center-folding runs on
        EVERY out-of-bounds note again (not just at statement-retile
        boundaries), which already supplies a restoring force on its own.
        That changes what this test can honestly assert: the
        directed_anchor_shift restoring pull (added separately, for the
        different case of a chord shorter than its motif's own cycle --
        see TestGenerateDevelopOpeningAnchorWallCheck) is no longer
        guaranteed to beat a no-pull baseline in every scenario, since
        center-folding alone is already doing real work here -- measured
        directly below (frac_with is not asserted less than frac_without
        anymore, because it isn't reliably true). What's still true, and
        still worth guarding: neither configuration collapses toward the
        severe concentration the original bug produced.
        """
        import intervals.music.melody as melody_mod
        ...
        # NOT asserting frac_with < frac_without anymore -- see docstring.
        # Both configurations now include per-note center-folding
        # (prefer_neighbor_fold is permanently off), so the anchor-nudge's
        # marginal contribution on top of that is no longer guaranteed
        # positive in every scenario. What matters is that NEITHER
        # collapses toward the original bug's severity.
        assert frac_with < 0.35, (
            f"top pitch still accounts for {frac_with:.2f} of the melody "
            f"with the restoring pull active -- concentration is back "
            f"to roughly the unfixed level"
        )
        assert frac_without < 0.35, (
            f"top pitch accounts for {frac_without:.2f} of the melody "
            f"even WITHOUT the restoring pull -- center-folding alone "
            f"(prefer_neighbor_fold permanently off) should already keep "
            f"this bounded; if this fails, center-folding itself may have "
            f"regressed, independent of directed_anchor_shift"
        )
