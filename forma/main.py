    ...
    for s in sections_to_display:
        ...
        if voices:
            for vi, v in enumerate(voices):
                motif_name = v.get("motif", "theme")
                species_str = f" species={v['species']}" if v.get("species") else ""
                print(f"      voice {vi+1}: register={v.get('register','mid')} "
                      f"behavior={v.get('behavior','lyrical')}{species_str} "
                      f"motif={motif_name} vel={v.get('velocity',64)}")
