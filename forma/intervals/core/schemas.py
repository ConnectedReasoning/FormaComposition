...
class VoiceModel(BaseModel):
    """
    A single peer voice in the voices array.
    ...
    by melody.py (behavior-driven generative / motif / lyrical).

    Each voice is fully independent: its own register, motif, rhythm, and
    velocity.  The engine generates them sequentially; each voice reads the
    snapshots of all previously generated voices so it can avoid collisions.

    ``register`` maps to MIDI pitch ranges:
        high     → G4–E6  (67–88)
        mid      → C4–A5  (60–81)
        low_mid  → C3–A4  (48–69)
        low      → C2–A3  (36–57)
    ``above`` / ``below`` are aliases accepted for counterpoint compat.
    """
    model_config = ConfigDict(extra="forbid", populate_by_name=True)
    ...
    # Motif — name from theme pool, or inline dict.  None → use theme primary.
    motif:      Optional[Union[str, dict]]                  = None

    # Rhythm — same sources as section.rhythm
    rhythm:     RhythmSourceLiteral                         = "free"
