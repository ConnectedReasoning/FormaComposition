...
class PieceContext:
    ...
    # Form-position weight tables for suggest_transform.
    # A composer states the subject first, develops it in the middle,
    # and returns to it near the end.  Keys are transform names; values
    # are multiplicative weights combined with the usage-penalty weights.
    _FORM_WEIGHTS_OPENING = {   # form_position 0.0–0.25
        "original": 2.0, "transpose_up": 2.0, "transpose_down": 2.0,
        "retrograde_inversion": 0.3, "augmentation": 0.3,
    }
    _FORM_WEIGHTS_MIDDLE = {    # form_position 0.25–0.75
        "inversion": 1.5, "retrograde": 1.5, "retrograde_inversion": 1.5,
        "augmentation": 1.5, "diminution": 1.5,
        "original": 0.5,
    }
    _FORM_WEIGHTS_ENDING = {    # form_position 0.75–1.0
        "original": 1.8, "augmentation": 1.8,
        "diminution": 0.4, "shuffle": 0.4,
    }

    @staticmethod
    def _form_position_weight(
        transform: str,
        section_index: int,
        total_sections: Optional[int],
    ) -> float:
        """Multiplicative weight for a transform based on where we are in
        the large-scale form.  Returns 1.0 (neutral) when total_sections
        is unknown, so existing call sites are unaffected."""
        if not total_sections or total_sections <= 0:
            return 1.0
        pos = section_index / total_sections
        if pos < 0.25:
            table = PieceContext._FORM_WEIGHTS_OPENING
        elif pos < 0.75:
            table = PieceContext._FORM_WEIGHTS_MIDDLE
        else:
            table = PieceContext._FORM_WEIGHTS_ENDING
        return table.get(transform, 1.0)
    ...
    def suggest_transform(
        ...
        2. Weighted random selection that penalizes recently-used transforms
           and (when total_sections is known) favors form-appropriate
           transforms: statement at the opening, development in the middle,
           return near the end.
        ...
        Args:
            ...
            total_sections:     Total sections in the piece (enables
                                form-aware weighting; None disables it)
        ...
        # Weighted random: penalize recent usage, weight by form position.
        ...
        for t in available:
            ...
            if self.transform_history and self.transform_history[-1] == t:
                w *= 0.25
            # Form-position weight (×1.0 when total_sections unknown)
            w *= self._form_position_weight(t, section_index, total_sections)
            weights.append(w)
