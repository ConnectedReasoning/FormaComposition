...
| `melody` | enum | no, default `"generative"` | `lyrical, generative, sparse, develop`. **Corrected:** earlier revisions of this doc also listed `motif` and `rhythmic` — neither is valid. `MelodyLiteral` permits only these four, and `BEHAVIOR_GENERATORS` dispatches only these four, so `"melody": "motif"` fails schema validation. |
...
| `name` | string | no | |
| `note_length_range` | object `{min, max, quantum?}` | no | **New.** Decouples note length from density for **melody + free-species counterpoint only**. When set, durations are sampled in `[min, max]` beats instead of the density grid; density then governs only rest frequency, not length. Applies to **all** melody behaviors (works at the rhythm layer below behavior). Harmony/bass untouched by design. **No-op under a `groove` or under `rhythm: "pattern"/"motif"`** — both flagged by lint. See resolved note #5. |
...
| `note_length_range` | object `{min, max, quantum?}`, optional | — | **New.** Per-voice override of the section's `note_length_range` (free species only). When set, this voice samples its own lengths; when omitted it inherits the section-level range if one exists. |
...
5. **Note-length range — `note_length_range`, new (melody + free-species counterpoint only).** Before this, note *length* was welded to `density`: at `medium`/`full`, the melody grid was uniform eighth notes — every note the same length, which reads as staccato/pointillistic no matter the behavior. `note_length_range` breaks that weld. Set `{"min": 0.5, "max": 3.0}` on a section and durations are sampled freely in that range instead of the grid, while `density` keeps its now-separate job of governing rest frequency (how busy the line is). Two orthogonal knobs where there was one. Omit the field and nothing changes — byte-identical, deterministic, backward-compatible.
   - **Scope is deliberate: lead voices only.** The sampler is gated to `voice_type == "melody"`, which covers the section melody line and free-species counterpoint. **Harmony and bass never sample it** — they stay grid-disciplined by design (the call sites pass `voice_type="bass"/"chord"`, which the range branch ignores). This was a design decision, not an oversight: restrictions for accompaniment, freedom for the lead.
   - **Applies to every melody behavior.** Because the range operates at the rhythm layer *below* behavior, `lyrical`/`generative`/`sparse`/`develop` all get it uniformly — you don't configure it per behavior. (Note this is also why the `develop` behavior's own motif-rhythm is still discarded for duration: the onset grid, now range-sampled, still wins. Same locked-grid root cause as the augmentation/diminution issue.)
   - **`quantum` is the freedom dial.** Sampled lengths snap to `quantum` (default `0.25`) so they stay grid-legible in Logic rather than landing on arbitrary floats like 1.37. `0.5` → eighth-legible; `0.25` → sixteenth; smaller → more fluid. This was a judgment call — pure continuous sampling sounds sloppy against the DAW grid, so snapping is on by default. If you want genuinely ungridded lengths for a specific texture, that's a one-line change to make `quantum: 0` mean "no snapping" — not currently wired.
   - **Two no-op traps, both surfaced by lint (not silent).** `note_length_range` is ignored (a) under a `groove` — the groove fully specifies durations, so groove wins; and (b) under `rhythm: "pattern"` or `rhythm: "motif"` — those sources precompute their own onset/duration grid that bypasses the sampler. In both cases the field does nothing, and in both cases the consumption linter now prints a named warning on the run rather than leaving you to wonder. Precedence, explicitly: **`groove` > `note_length_range` > `density` grid**, and range needs `rhythm: "free"`.
   - **Per-voice counterpoint override.** A `CounterpointModel` can carry its own `note_length_range` (with its own `quantum`); when present it overrides the section-level range for that voice, when absent it inherits the section's. Free species only — strict species (first/second/etc.) keep their classical note-against-note conventions regardless.
   - **Bounds.** `NoteLengthRangeModel`: `min` and `max` both `> 0`, `max >= min` (else schema error), `quantum > 0` default `0.25`. `extra="forbid"`, so a typo'd key inside the object is caught, not silently swallowed.
