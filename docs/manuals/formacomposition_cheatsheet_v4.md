...
by inference):
1. `harmony_rhythm.note_duration` was **removed** from the schema. It's now a hard
   validation error (`extra="forbid"`), not a silent no-op.
2. Invalid `species` (`second`/`third`/`fourth`/`fifth`) now fails at **validation time**
   (a Pydantic field validator), not at render time. Same for the counterpoint (3) and
   voices (4) hard caps — both are now clean validation errors, not a deep `IndexError`.
3. Bass swing handling was refactored: it's now a single centralized post-pass applied to
   **all 8** bass styles, not gated by style name. The audible outcome for your existing
   pieces is unchanged (6 styles still produce no off-beat onsets for it to act on), but the
   mechanism is different — worth knowing if you ever add a configurable subdivision to
   `pulse`, since that would suddenly make swing audible there too.
...
| `key` | string | **yes** | theme's key now lives directly on the piece (single-file format — theme merged in; see `ThemeModel` retirement note). Any non-empty string, not enum-checked here. |
| `mode` | string | **yes** | same — merged from theme, any non-empty string, not enum-checked here. |
| `motif` / `motifs` | see MotifModel | one of the two, optional | merged from theme; same rules as the theme-side table above |
...
| `tempo` | int **or** object `{min, max}` | no | Accepts either a bare int (auto-coerced into a fixed-value `{min, max}` range, `min==max`) or the theme-style range object directly. If omitted, warns and falls back to a 120bpm default via `resolved_tempo()`. Range 20–300 either way. |
| `title` | string | no | |
...
**One file per piece now.** `ThemeModel` has been retired; there's no more paired
`theme_*.json` + `piece_*.json` loading — `key`/`mode`/`motif`/`tempo` all live directly
on the single piece file, alongside `title`/`sections`/`form`. `main.py`'s `load_song()`
only ever reads one file. If you're starting a new piece from an old paired-file
template, merge the theme fields onto the piece top level first — a file missing `key`/
`mode` now fails validation outright (`Field required`) rather than silently degrading.
...
| `bass_style` | enum | no, default `"root_fifth"` | `root_fifth, walking, pedal, root_only, melodic, steady, pulse, motif` (8 styles). `steady` = a locked figure from `STEADY_FIGURES`, re-approached each chord change. `motif` reuses the theme's `intervals`+`rhythm`, re-anchored to each chord's root, snapping to the nearest chord tone (root/3rd/5th) within a whole step, else nearest scale tone; **requires a theme motif with both `intervals` and `rhythm`** or it falls back to `root_only` with a `UserWarning`. **Swing gotcha:** swing is applied as a single centralized pass across **all 8** bass styles (`bass.py`'s `_apply_swing_to_bass`) — it's no longer gated by style name. In practice it still only produces an audible effect for `melodic` and `motif`, because the other 6 styles' figures never place a note off the beat (nothing on the grid for the swing offset to act on) — same audible outcome as before, different mechanism. If you ever give `pulse` a sub-quarter-note subdivision, it would start swinging too; there's no schema field for that yet. **No separate rhythm layer** — timing comes entirely from `bass_style`; there's no `bass_rhythm` block. **Collision, fixed:** if both `rhythm: "motif"` (a generic bass rhythm-anchor override, unpitched) and `bass_style: "motif"` (pitched) are set on the same section, `bass_style` now wins. |
...
| `counterpoint` | object or array | no | up to 3 voices — **more than 3 is a validation-time error** (a Pydantic field validator on `SectionModel`, not a deep render-time crash); bare object auto-wrapped into a 1-item list. **`voices[]` supersedes `counterpoint` outright if both are present on a section.** See the dedicated tables below — and note the `species` crash risk (only `first`/`free` implemented, and it now fails at validation time too — see Enum reference). |
...
| `fugal_techniques` | dict | no | untyped. Unrelated to `harmony_rhythm.transform_imitation` despite the conceptual overlap — different mechanism, same-sounding idea, don't conflate them. **`canon_interval` is a no-op unless `canonic_imitation: true` is also set** — `generate_melody_for_progression()` reads `canon_interval` unconditionally (default 4) but only applies the shift-and-trim inside `if canonic_imitation and canon_interval > 0`. This is lint-checked. |
...
| `harmony_pattern` | object | conditional | required if `harmony_rhythm.rhythm: "pattern"` **is set explicitly**. Gap: if `harmony_rhythm` is entirely unset and the section just *inherits* `rhythm: "pattern"` from `section.rhythm`, there's no schema check and no `harmony_pattern` block — harmony renders completely silent (no events, no print, no error). This inherited case is lint-checked; the explicit case was already schema-enforced. |
...
| `melodic_variation` | enum | no | `isorhythmic` — only meaningful when `rhythm: "motif"` **and** the theme's pool has >1 motif **and** the lead voice has no motif override of its own (any one of these three alone makes it inert; lint-checked — this specific check needs the theme passed to `lint_piece(piece, theme=...)`, since it's the only check that requires resolving the motif pool size). Default (unset): pitch is pinned to the same motif driving the rhythm. `"isorhythmic"`: rhythm stays anchored to one motif while pitch is redrawn from a different pool member each repetition — fixed talea, varying color; opt-in only. |
| `melody` | enum or object | no, default `"generative"` | `lyrical, generative, sparse, develop`. **Gotcha, two different defaults:** omit `melody` entirely → defaults to `"generative"`. Give it as a dict (or use `voices[0]`) and omit `behavior` inside → defaults to `"lyrical"` instead (`VoiceModel.behavior`'s own default). Same field, two fallback values depending on which form you used. Behavior meanings (pitch selection logic): `lyrical`/`generative` = fresh weighted random draw per note. `develop` = continuously restates/re-transforms the motif cell across the entire chord/section (falls back to `generative` if no motif at all); **verified wired for the lead voice only** — a peer voice (`voices[1:]`) with `develop` is a silent no-op, not even lint-checked, rendering as if `generative`. `sparse` = same pool as lyrical, thinner. Dict form also accepts `register`/`velocity`. **Cross-section continuity:** every section after the first biases its melody's opening note toward or away from the previous section's ending note, based on that section's `arc` (ascending-ending + `swell`/`build` → continues upward; ascending-ending + `fade`/`fade_out`/`decay` → reverses downward; descending-ending + `build` → reverses upward; otherwise no bias). Always on, not disableable — means two `exact_repeat` occurrences of the same section will **not** necessarily open on the same note if preceded by different sections. |
...
| `groove` | string, optional | must be a valid `GROOVES` key. Inert under `"sustain"` (no onset pattern) **and** under `"motif"` (the motif cell supplies its own fixed onset grid) — only audible under `"free"`. **Lint-checked for the `"motif"` case only** — the `"sustain"` case is a real coverage gap: setting `groove` alongside `rhythm: "sustain"` is schema-legal, dead, and currently produces no lint warning at all. (`piece_long_amen.json` has this exact combination in all 4 sections right now.) |
...
| ~~`note_duration`~~ | — | **Removed from the schema.** `HarmonyRhythmModel` is `extra="forbid"`, and this field is no longer declared — setting it now raises a hard validation error instead of silently doing nothing. Chord length comes from the rhythm source instead (`sustain` holds the harmonic span; `motif` takes durations from the motif cell). If any older piece file sets this, it will fail validation and needs the field removed. |
| `rhythm` | enum, optional | `motif, pattern, sustain, free` (cascades: `harmony_rhythm.rhythm` → `section.rhythm` → `"free"` if all omitted). **Correction: `"motif"` is NOT retired** — it was pulled for one release (the old implementation just borrowed melody's motif rhythm verbatim), then reintroduced as a real, independent mechanism. `sustain` = one held note per chord span, zero internal movement — **the** lever for "does this section have any internal motion at all," independent of progression variety, `chord_bars` shaping, groove, or density. `free` = density-grid-driven, can re-articulate within a chord. `pattern` needs a hand-authored `harmony_pattern` block or it's a hard validation error **only when set explicitly** — see the `harmony_pattern` row above for the inherited-rhythm gap. |
...
| `species` | enum | `"free"` | `free, first, second, third, fourth, fifth`. **Only `first` and `free` are actually implemented.** Setting `second`/`third`/`fourth`/`fifth` now fails **at validation time** — a Pydantic field validator raises `ValueError` ("not implemented in counterpoint.py... choose 'first' or 'free'") the moment the piece is validated, before generation ever starts. (Previously this passed validation cleanly and only crashed deep in `generate_counterpoint()` at render time — that's fixed now.) `lint.py`'s own no-op check for this still exists but can no longer actually fire, since validation blocks first. |
| `velocity` | int | 58 | 1–127 |

**Cap, and when it's enforced:** more than 3 counterpoint voices is a **validation-time**
`ValueError` (a `SectionModel` field validator: "counterpoint supports at most 3 voices"),
not a deep render-time crash.
...
| `species` | enum, optional | — | if present → `counterpoint.py` path instead of `melody.py`; `above`/`below` is *derived* from whether this voice's register sits above/below the lead's, not independently settable. Same second/third/fourth/fifth crash risk as `counterpoint[]` applies, including the same validation-time-not-render-time fix. |
| `velocity` | int | 64 | 1–127 |

**4-voice hard cap, and when it's enforced:** more than 3 peer voices (4 total including
lead) is now a **validation-time** `ValueError` ("voices supports at most 4 total"), raised
by a `SectionModel` field validator before generation starts. (Previously this passed
validation and only crashed deep in MIDI writing — `generator.py` writes peer voices to
3 pre-built track slots and indexes past the end of that list on a 4th peer. Same class of
fix as the counterpoint cap above; catch is intentionally symmetric.)
...
chord.** (Still true in the current `piece_broadway_boogie_v8.json` — same 12-chord
`m1_grid_1`.) This is currently harmless because the only randomness that seed feeds
(rest-thinning) is an explicit no-op under `harmony_rhythm.rhythm: "sustain"`, which is
what `m1_grid_1` uses. **It would become audible the moment that section's harmony source
changes to `"free"`, `"motif"`, or gets a hand-authored `"pattern"`** — worth remembering
if you ever revisit that section's harmony treatment. Until the seeding gets a proper fix,
keep progressions ≤10 chords per section for any harmony source other than `sustain`.
...
- **Transform**: `original, inversion, retrograde, retrograde_inversion, augmentation, diminution, transpose_up, transpose_down, shuffle, expand, compress, sequence` — 12 total. Of these, only `inversion, retrograde, shuffle, sequence` vary pitch in `develop`; see the Motif table above for the full breakdown of what each one actually does (several are rhythm-only or dead).
- **Counterpoint species**: `free, first, second, third, fourth, fifth` — only `free`/`first` actually implemented; the rest now fail **at validation time** (moved from a render-time crash — see the `counterpoint[]`/`voices[]` tables above).
...
| A counterpoint voice with real classical species rules | `species: "first"` or `species: "free"` **only** — anything else now fails at validation time, before generation starts |
| Swing that's actually audible on bass | `bass_style: "melodic"` or `"motif"` — the other 6 styles still produce no off-beat onsets for the swing pass to act on, even though it now runs uniformly across all 8 |
...
| A dead `harmony_rhythm.groove` you won't get warned about | Don't set `groove` alongside `rhythm: "sustain"` — it's inert and, unlike the `rhythm: "motif"` case, **not currently lint-checked** |
