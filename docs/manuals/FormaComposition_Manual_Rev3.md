...
|---|---|
| theme.json | Key, mode, tempo range, seed motif or phrase, colour palette |
...
```json
{
  ...
  // Option B — prosodic rhythm (requires prosodic_rhythm: true)
  "prosodic_rhythm": true,
...
| phrase | string | — | English phrase used for pitch contour (pitch motif only) or, when combined with `prosodic_rhythm: true`, for full rhythmic driving of all voices (see §6) |
| prosodic_rhythm | boolean | — | Enable phrase-driven rhythm template. Requires `phrase` and the `pronouncing` library. Fails hard if either is absent. (see §6) |
...
4. **Prosodic lens** — if `prosodic_rhythm: true` and `phrase` are set in theme
...
The prosody system has two distinct modes with different purposes and
different requirements. Understanding which one you want is important —
they are configured differently and produce different results.

### 6.1 — Mode A: Phrase → Pitch Motif (pitch contour only)

Set `"phrase": "your words"` in theme.json *without* `prosodic_rhythm`.
The engine converts syllable stress into a harmonically-aware pitch motif
using the CMU Pronouncing Dictionary. This motif drives **pitch selection
only** — it has no effect on rhythm.

This mode works even if `pronouncing` is not installed, falling back to a
syllable-count heuristic. Results with the fallback are approximate; install
`pronouncing` for accurate stress analysis.
...
> wins and `phrase` is ignored entirely. Use one or the other.

### 6.2 — Mode B: Phrase → Rhythmic Template (all voices)

Set both `"phrase"` and `"prosodic_rhythm": true` in theme.json. The
engine analyzes syllable stress, onset timing, and vowel duration to
build a `RhythmicTemplate` that drives rhythm for all voices via lens
functions:

| **Voice** | **Lens** | **Behaviour** |
|---|---|---|
| Melody | melody_lens | Every syllable onset. Full articulation — the melody pronounces the phrase. |
...
```json
{
  ...
  "tempo": { "min": 62, "max": 66 },
  "prosodic_rhythm": true,
  "phrase": "The forgotten harbor sleeps"
}
```
...
- `phrase` must be present alongside `prosodic_rhythm: true`
- Generation fails with a hard error if either is missing — there is no
  silent fallback for the rhythmic template mode

**Why no fallback?** Without the CMU dictionary, every syllable is treated
as a primary stress (`stress_pattern = "111..."`), producing a uniform rhythm
that bears no relation to natural speech. The engine refuses to generate
rather than produce output that appears prosodic but isn't.

**Per-word warnings:** If the `pronouncing` library is installed but specific
words in the phrase are not in the CMU dictionary (proper nouns, brand names,
neologisms), the validator emits a `[WARN]` listing those words. Stress for
those words is estimated by syllable count. Generation proceeds.

### 6.3 — Tension Hierarchy (Mode A)

When `phrase` drives pitch selection (Mode A), tension levels govern which
chord tones are available at each stress position:
...
### 6.4 — Section Properties That Drive Tension
...
- `prosodic_rhythm: true` without `phrase` → **hard error**
- `prosodic_rhythm: true` without the `pronouncing` library installed → **hard error** (install with `pip install pronouncing`)
- `prosodic_rhythm: true` with words not in CMU dictionary → warning listing the affected words; generation continues with syllable-count estimates for those words
...
| theme has prosodic_rhythm=true but no 'phrase' | Add a phrase to theme.json alongside prosodic_rhythm |
| 'pronouncing' library not installed | Run: pip install pronouncing |
| phrase contains words not in CMU dictionary | Warning only — generation continues. Those words use syllable-count stress estimates. Consider rewording if rhythm feels wrong. |
